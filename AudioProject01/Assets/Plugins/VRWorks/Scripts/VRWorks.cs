        ...
        public void OnPreRender()
        {
            ...
            if ((renderFeatures & kPluginExtFeatureMRSActive) != 0)
            {
                if (!m_Camera.stereoEnabled)
                {
                    ...
                    Shader.SetGlobalVector("_nvScaleOffset", new Vector4(0.7f, 0.7f, 0, 0));
                }
                else
                {
                    Shader.SetGlobalVector("_nvScaleOffset", new Vector4(1.0f, 1.0f, 0, 0)); 
                }
    ...
    {
      {
        ...
        string plguinMRSKeyword = "VRWORKS_MRS_OFFSETS";
        ...
        if ((m_RenderFeatures & kPluginExtFeatureMRSActive) != 0)
        {
            m_CommandBuffer[(int)CB.kCB_BeforeForward].EnableShaderKeyword(plguinMRSKeyword);
            m_CommandBuffer[(int)CB.kCB_AfterForward].DisableShaderKeyword(plguinMRSKeyword);
        }
